...
#include <vector>
/*
std::bind works as a Functional Adaptor i.e. it takes a function as input and
returns a new function Object as an output with with one or more of the
arguments of passed function bound or rearranged (partial function application).
*/

// https://www.youtube.com/watch?v=ZlHi8txU4aQ

template <typename T> void printTemplate(T value) {
  std::cout << value << std::endl;
}

void print(int firstParam, int secondParam) {
  std::cout << "First Param is: " << firstParam
            << " Second Param is: " << secondParam << std::endl;
}
...
int main() {
  ...
  { // first example
    ...
    auto secondPrinter =
        std::bind(&print, std::ref(firstParam), std::ref(secondParam));
  ...
  { // second example
    auto reversePrintFunc =
        std::bind(&print, std::placeholders::_2, std::placeholders::_1);
