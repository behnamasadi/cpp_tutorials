...
namespace tag_dispatching
{
    ...
    {
        return equal( lhs, rhs ,std::conditional_t<std::is_floating_point<T>::value,std::true_type,std::false_type >{} );

        //return equal( lhs, rhs ,typename std::conditional<std::is_floating_point<T>::value,std::true_type,std::false_type >::type{} );
...
template <typename T>
bool equal(T lhs, T rhs,  std::enable_if_t<! std::is_floating_point<T>::value>* =nullptr )
{
