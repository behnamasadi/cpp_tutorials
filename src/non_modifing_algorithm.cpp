...
void unique()
{
    ...
    last_it=std::unique(vec1.begin(),vec1.end());//vec1->{1,3,5,7,x,x}
...
void min_max()
{
    ...
    std::cout<<"the absoulute max is: " <<*abs_max<< std::endl;
...
void transform()
{
    ...
    std::transform(vec1.begin(),vec1.end(),vec2.begin(),[](int x){return x++;});//vec2=vec1+1
...
void generate()
{
    //    std::generate(vec1.begin(),vec2.end(),rand);
}
