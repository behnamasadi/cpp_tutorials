...
std::invoke takes something callable, and arguments to call it with, and does
the call. std::invoke( f, args... ) is a slight generalization of typing
f(args...) that also handles a few additional cases.
...
int subtractor(int x, int y) { return x - y; }

// int caclculator(int a, int b, int (*calcus_operation)(int,int)) is also
// correct
...
int calculator(int x, int y, mathOperations &c,
               int (mathOperations::*func_ptr)(int, int)) {
...
int calculator(
    int x, int y, PointertoMemberFunction::mathOperations &c,
    std::function<int(PointertoMemberFunction::mathOperations &, int, int)>
        calcus_operation) {
...
/*
templates are a compile-time feature and are a design tool for compile-time
polymorphism. If runtime dynamic behaviour is to be achieved through callbacks,
templates will help but they won't induce runtime dynamics.
*/
...
int main() {
  ...
  /*************************** 1)Function Pointer ****************************/
  {
    ...
    int (*adder_fn_ptr)(int, int) =
        FunctionPointer::adder; // int(*adder_fn_ptr)(int,int) =&adder; is also
                                // okay
    ...
    FunctionPointer::caclculator_fn_ptr subtractor_fn_ptr =
        FunctionPointer::subtractor;
...

  {
    ...
    c = PointertoMemberFunction::calculator(
        a, b, my_mathOperations,
        &PointertoMemberFunction::mathOperations::adder);

    std::cout << c << std::endl;
  }

  /**************************** 3) std::function ****************************/
  // The std::function class is a polymorphic function wrapper to store, copy or
  // invoke callables.

  {
    ...
    std::function<int(PointertoMemberFunction::mathOperations & C, int, int)>
        std_func_class_member_adder =
            &PointertoMemberFunction::mathOperations::adder;
    ...
    c = stdFunction::calculator(
        a, b, C, &PointertoMemberFunction::mathOperations::adder);
    std::cout << c << std::endl;
    ...
    c = stdFunction::calculator(a, b,
                                std::bind(FunctionPointer::adder,
                                          std::placeholders::_1,
                                          std::placeholders::_2));
    std::cout << c << std::endl;
