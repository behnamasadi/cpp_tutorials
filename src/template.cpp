...
of TYPE would be deduced by compiler depending on the argument passed to this
function. When you attach template with a generic function, compiler would
compile only for required set of data-types!.
...
I)Pointers, References and Arrays with Templates
        ...
        */
template <class T> double GetAverage(T tArray[], int nElements) {
  ...
  It essentially means: Call the default constructor for this datatype. For int,
  it would be:

          int tSum = int();

  Which effectively initializes the variable with 0. Similarly, for float, it
  would set this variable to 0.0f. Though not covered yet, if a user defined
  class type comes from T, it would call the default constructor of that class
  (If callable, otherwise relevant error). As you can understand that T might be
  any data-type, we cannot initialize tSum simply with an integer zero (0). In
  real case, it may be a some string class, which initializes it with empty
  string ("").
  */
...
void TwiceIt(T& tData){}
 ...
 At this point, you now clearly understand that template parameter type T may be
inferred from T&, T* or T[].
 */
...
/*
 The default-ness may only go from right to left, meaning, if nth argument is
 required to be default, (n+1)th must also be default, and so on till last
...
template <typename T> void add(T a, T b) { std::cout << a + b << std::endl; }
template <> void add(std::string a, std::string b) {
...
It's perfectly possible to template a class on an integer rather than a type. We
can assign the templated value to a variable, or otherwise manipulate it in a
way we might with any other integer literal:
...
What you have there is of the last kind. It's a compile time constant (so-called
constant expression) and is of type integer or enumeration. After looking it up
in the standard, i had to move class templates up into the types section - even
though templates are not types. But they are called type-parameters for the
purpose of describing those kinds nonetheless. You can have pointers (and also
member pointers) and references to objects/functions that have external linkage
(those that can be linked to from other object files and whose address is unique
in the entire program). Examples:
...
Pool<allocator> test;
A template without any parameters is not possible. But a template without any
explicit argument is possible - it has default arguments:
...
Vector<> test;
Syntactically, template<> is reserved to mark an explicit template
specialization, instead of a template without parameters:
