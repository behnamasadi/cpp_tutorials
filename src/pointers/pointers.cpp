...
public:
  ...
  ~person() {
    std::cout << ((id > -1)
                      ? std::string("person with ") + std::to_string(this->id) +
...
void rawPointerExample() {
  ...
  /*	here we define a pointer variable, it is located somewhere in the memory
     that we don't know the value of that, it has some random value
  */
  int *pnPtr;
  // is a new keyword introduced in C++11. nullptr is meant as a replacement to
  // NULL
  pnPtr = nullptr;

  int nValue = 5;
  /*	here during compliation time, nValue get address and we assign  that
   * address to our pointer variable */
  pnPtr = &nValue;
  ...
  /*
     ...
     a 32-bit computer uses 32-bit memory addresses — consequently, a pointer on
     a 32-bit machine is 32 bits (4 bytes). On a 64-bit machine, a pointer would
     be 64 bits (8 bytes).this is true regardless of what is being pointed to
  */
  ...
  std::cout << "sizeof(postEmployees[0])= " << sizeof(postEmployees[0])
            << std::endl;
...
void weakPointerExample() {
  ...
  std::cout << "sharedEntity0 is still alive because of person0=sharedEntity0"
            << std::endl;

  // if we change the above code to the following the object will die after the
  // {} block
  ...
  std::cout
      << "sharedEntity1 died befor reaching here since person1 is a weak_ptr"
      << std::endl;
}
...
void sharedPointerExample() {
  ...
  /*  when  you erase items in a vector the occupied space will be freed, but if
   you have stored pointers inside a vector you have to manualy call delete
   function to free the space.

   The destructor for std::vector<T> ensures that the destructor for T is called
   for every element stored in the vector.
  */
