...
struct S {
  ...
  // Use *this from within a function of a class to get a reference to the
  // current object
...
public:
  ...
  */
  ...
  /* In C++0x, the compiler chooses between the copy constructor and the move
   * constructor based on whether the argument to the assignment operator is an
   * lvalue or an rvalue.
   */
...
struct noncopyable {
  ...
  // Move construction and move assignment must be enabled explicitly, if
  // desiered.
...
int main() {
  ...
  money3.data[0] = -1;
  money5.data[2] = -2;
  std::cout << "money5 after updating money5.data[2]=-2 : " << money5
            << std::endl;
  std::cout << "money3 after updating money3.data[0]=-1 : " << money3
            << std::endl;

  money money6(money5);
  money5.data[0] = -3;
  money6.data[2] = -4;
  std::cout << "money5 after updating money5.data[2]=-3 : " << money5
            << std::endl;
  std::cout << "money6 after updating money3.data[0]=-4 : " << money6
            << std::endl;
