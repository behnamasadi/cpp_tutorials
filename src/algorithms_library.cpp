...
void unique()
{
    ...
    std::vector<int>::iterator last_it=std::unique(vec1.begin(),vec1.end()); //==> 2,1,5,2,3,1,7
...
void count()
{
    ...
    int larger_than_five=std::count_if(numbers.begin(),numbers.end(),[](int x){return x>5;});
    std::cout<<"there is (are) " <<larger_than_five << " number larger than 5"<<std::endl;
...
void min_max()
{
    ...
    std::vector<int>::iterator  abs_max=std::max_element(numbers.begin(),numbers.end() );
    std::cout<<"the absolute max is: " <<*abs_max<< std::endl;


    std::vector<int>::iterator  first_digit_max=std::max_element(numbers.begin(),numbers.end(),[](int x,int y){return (x%10)<(y%10);} );
    std::cout<<"the number with first digit max is : " <<*first_digit_max<< std::endl;

    std::pair<std::vector<int>::iterator,std::vector<int>::iterator> min_max= std::minmax_element(numbers.begin(),numbers.end());
...
void find()
{
    ...
    std::vector<int> numbers={2,3,1,0,-4,7,5,3};
    int number_three=3;
    std::vector<int>::iterator find_it= std::find(numbers.begin(),numbers.end(),number_three);
    if (find_it != std::end(numbers))
    {
        ...
        std::ptrdiff_t pos = std::distance(numbers.begin(), find(numbers.begin(), numbers.end(), number_three));
...
void search()
{
    ...
    int number_three=3;
    std::vector<int>::iterator find_it=std::search_n(numbers.begin(),numbers.end(),how_many_three,number_three);
    std::cout << "the first consecutive occurrence of three number two times located at " << find_it-numbers.begin() << '\n';


    std::vector<int> sub_range={0,-4,7};
    find_it=std::search(numbers.begin(),numbers.end()  ,sub_range.begin(),sub_range.end() );
    std::cout << "the first consecutive occurrence of 0,-4,7 located at " << find_it-numbers.begin() << '\n';


    std::vector<int> items={87,5,43};
    find_it=std::find_first_of(numbers.begin(),numbers.end() ,items.begin(),items.end());
    std::cout << "the first occurrence of any 87,5,43 located at " << find_it-numbers.begin() << '\n';
...
void equal()
{
    ...
    if(std::is_permutation(numbers.begin(),numbers.end(),numbers_replicate.begin()))
    {
...
void mismatch()
{
    ...
    std::pair< std::vector<int>::iterator, std::vector<int>::iterator > mis_matches= std::mismatch(numbers.begin(),numbers.end(),numbers_changed.begin());
    std::cout << "Mismatch at " << (mis_matches.first - numbers.begin()) << ". Because: " << *mis_matches.first << " != " << *mis_matches.second << std::endl;
}
...
void partition()
{
    ...
    auto partition_it = std::partition(v.begin(), v.end(), [](int i){return i % 2 == 0;});

    std::cout << "\nPartitioned vector:\n    ";
    std::copy(std::begin(v), partition_it, std::ostream_iterator<int>(std::cout, " "));
    std::cout << " * ";
    std::copy(partition_it, std::end(v), std::ostream_iterator<int>(std::cout, " "));
    std::cout << '\n';
...
void copy()
{
    ...
    std::copy_if(vec1.begin(),vec1.end(),vec2.begin(),[](int x){return x%2==0;});
...
*/
    ...
    std::transform(vec1.begin(),vec1.end(),vec2.begin(),[](int x){return x+1;});//vec2=vec1+1
    std::transform(vec1.begin(),vec1.end(),vec2.begin(),vec3.begin() ,[](int x,int y){return x+y;});//vec3=vec1+vec2
}

void swap()
{
    std::vector<int> vec1={4,12,3,6,};
...
void replace()
{
    ...
    std::replace_if(vec1.begin(),vec1.end(),[](int x){return x>2;},-1);//if values are bigger 2, they will be replaced by -1
}
...
void remove()
{
    ...
    std::remove_if(vec1.begin() ,vec1.end(), [](int x){return x%2==0;} );//removes even number from vec1
}
...
void lexicographical_compare()
{
    ...
    bool number_set1_less_than_number_set1=std::lexicographical_compare(number_set1.begin(),number_set1.end(),number_set2.begin(),number_set2.end());

    if(number_set1_less_than_number_set1)
    {

        for (auto c : number_set1) std::cout << c << ' ';
        std::cout << "< ";
        for (auto c : number_set2) std::cout << c << ' ';
        std::cout << '\n';
    }
    else
    {
    ...
    number_set1_less_than_number_set1=std::lexicographical_compare(number_set1.begin(),number_set1.end(),number_set2.begin(),number_set2.end());
...
void sort()
    ...
    bool is_number_set1_sorted_descendingly=std::is_sorted(number_set1.begin(),number_set1.end(),[](int x, int y){return x>=y;});
    ...
    std::vector<int>::iterator the_vector_is_sorted_unitil= std::is_sorted_until(number_set1.begin(),number_set1.end());
    std::cout<< *the_vector_is_sorted_unitil <<std::endl;
    std::cout<<"The array is sorted until element at: " <<std::distance(number_set1.begin(), the_vector_is_sorted_unitil  )<<std::endl;


    std::shuffle(vec1.begin(), vec1.end(), g);

    ///////////////////partial_sort, finding top 3 students, sorted///////////////////
    std::partial_sort(number_set1.begin(),number_set1.begin()+3,number_set1.end());
    ...
    /////////////////////nth_element, finding top 3 students, not necessarily sorted/////////////////////
    std::shuffle(vec1.begin(), vec1.end(), g);
    std::nth_element(number_set1.begin(),number_set1.begin()+3,number_set1.end());
    for(int i:number_set1)
        std::cout<<i<<" ";
    std::cout<<"\n";
    ...
    ///////////////////////partion data into two groups, neither of the groups are sorted/////////////////////
    std::shuffle(number_set1.begin(), number_set1.end(), g);
    std::vector<int>::iterator iterator_to_the_first_element_of_the_second_group= std::partition(number_set1.begin(),number_set1.end(),[](int x){return x<30; });

    std::cout<<"Element of the first group"<<"\n";
    for(std::vector<int>::iterator it=number_set1.begin();it!=iterator_to_the_first_element_of_the_second_group;it++ )
    {
        std::cout<<*it<<" ";
    }
    std::cout<<"\n";
    ...
    for(std::vector<int>::iterator it=iterator_to_the_first_element_of_the_second_group;it!=number_set1.end();it++ )
    {
        std::cout<<*it<<" ";
    }
    std::cout<<"\n";
    ...
    /////////////////////////stable_partition,partion data into two groups, but it preserve the original order///////////////////////
    std::shuffle(number_set1.begin(), number_set1.end(), g);
    std::stable_partition(number_set1.begin(),number_set1.end(),[](int x){return x<30; });
...
void binary_search()
{
    ...
    bool number_exist= std::binary_search(vec1.begin(),vec1.end(),number_to_search_for);
}
...
void includes()
{
    ...
    bool found=std::includes(vec1.begin(), vec1.end(),elements_to_search.begin(),elements_to_search.end()   );
...
{
// find the first position where an item can be iserted while still keep the sorting
...
//    std::sort(vec1.begin(),vec1.end(),std::greater<int>() );
    ...
    std::vector<int>::iterator  first_location_of_number_to_be_inserted, last_location_of_number_to_be_inserted;
    first_location_of_number_to_be_inserted=std::lower_bound(vec1.begin(),vec1.end(),number_to_be_inserted  );
    last_location_of_number_to_be_inserted=std::upper_bound(vec1.begin(),vec1.end(),number_to_be_inserted  );


    std::cout<<"First location that "<<number_to_be_inserted<<" can be inserted"<<std::endl;

    std::cout<<std::distance(vec1.begin(),first_location_of_number_to_be_inserted)<<std::endl;


    std::cout<<"Last location that "<<number_to_be_inserted<<" can be inserted"<<std::endl;
    std::cout<<std::distance(vec1.begin(),last_location_of_number_to_be_inserted)<<std::endl;
...
/*
you should send the object by ref to mak any changes and return parameters doesn't override the original object
*/
void for_eachExample()
{
    ...
    std::for_each(std::begin(my_vec), std::end(my_vec),[](int a){return a*3;});
    printArray(my_vec);
...
/*
    ...
    transform();
    copy();
    move();//--> not done yet, should be done for complicated data structures and classes
    swap();
    fill();
    generate();
    replace();
