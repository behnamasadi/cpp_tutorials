...
let say you want to write a piece of code that swap the bytes of integer type int, long int, unsigned int, etc.
One simpleimplemention wouldbe the following:
*/
...
/*
But here we don't check the type so we don't know if user has passed a double of integer,
so one solution is template specialization.
However, what happens if the caller passes in a float? Or an unsigned char? Or a pointer? You can quickly see how such a simple function would balloon out into many specializations, all to protect the caller.
*/
...
/*
A type trait is a way for you to get information about the types passed in as template arguments, at compile time, so you can make more intelligent decisions. The basics behind a type trait are:

1)You use a templated structure, usually named with the type trait you are after. Eg) is_integer, is_pointer, is_void
2)The structure contains a static const bool named value which defaults to a sensible state
3)You make specializations of the structure representing the traits you want to expose, and have those set their bool value to a sensible state
